...
Key consequence: **edits made in the admin are per-browser and do not touch any file.** To publish them, the user exports `portfolio-data.json` (Admin → *Exportieren*, `Store.exportAll()`) and commits it next to `index.html`. On load, `Store.loadPublished()` fetches that JSON and it becomes the content source, so the resolution order for every read is **localStorage → `portfolio-data.json` (PUBLISHED) → `DEFAULT_*` constants**. `loadPublished()` fails silently over `file://` (fetch is blocked there), so locally the site relies on localStorage/defaults; the JSON path only matters when served over http (GitHub Pages). Both `main.js` and `admin.js` `await Store.loadPublished()` before their first read. Editing the `DEFAULT_*` constants in `store.js` is still a valid way to set baseline content — there is no server or database.
